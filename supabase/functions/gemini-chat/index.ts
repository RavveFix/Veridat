...
import {
  ...
  extractInvoiceByCustomerName,
  detectAgentNeeds,
  shouldSkipHistorySearch,
  searchConversationHistory,
  getRecentConversations,
  formatHistoryResponse,
  formatWebSearchContext,
  triggerMemoryGenerator,
  generateSmartTitleIfNeeded,
  fetchWebSearchResults,
} from "./history.ts";
import {
  buildSkillAssistSystemPrompt,
  extractSkillDraft,
} from "./skill-assist.ts";
import {
  lookupCompanyOnAllabolag,
  callFortnoxRead,
  resolveSupplierNumber,
  createFortnoxWriteCaller,
  executeFortnoxTool,
  FORTNOX_READ_TOOLS,
  FORTNOX_WRITE_TOOLS,
  ALL_FORTNOX_TOOLS,
  ACCOUNTING_TOOL_RESPONSE_NAMES,
} from "./fortnox-tools.ts";
import {
  loadMemoryContext,
...
// [Types, utility functions, voucher helpers extracted to types.ts, utils.ts]

// [Memory system, history functions, skill-assist, types extracted to memory.ts, history.ts, skill-assist.ts, types.ts]

// [Fortnox tools, web search extracted to fortnox-tools.ts, history.ts]
                ...
                try {
                  ...
                  switch (action.action_type) {
                    case "create_supplier_invoice": {
                      ...
                      resultText =
                        `Leverantörsfaktura skapad som utkast (nr ${givenNumber})${siAttachmentNote}. Gå in i Fortnox → Leverantörsfakturor, kontrollera att allt stämmer och klicka Bokför.`;
                    ...
                    case "register_payment": {
                      ...
                      if (payType === "supplier") {
                        // Supplier payments removed — user books and pays manually in Fortnox
                        resultText = `Leverantörsfakturan hanteras som utkast i Fortnox. Bokför och betala manuellt under Leverantörsfakturor.`;
                        break;
                      }
                      // Customer payment
                      try {
              ...
              if (toolCallDetected) {
                ...
                try {
                  if (toolName === "conversation_search") {
                    ...
                    // Customer payment only — supplier payments removed (draft-only flow)
                    ...
                    if (payArgType === "supplier") {
                      toolResponseText = `Leverantörsfakturan hanteras som utkast i Fortnox. Bokför och betala manuellt under Leverantörsfakturor.`;
                    } else {
                      try {
                        const result = await callFortnoxWrite(
                          "registerInvoicePayment",
      ...
      try {
        switch (tool) {
          ...
          case "register_payment": {
            ...
            if (payArgType === "supplier") {
              // Supplier payments removed — user books and pays manually in Fortnox
              responseText = `Leverantörsfakturan hanteras som utkast i Fortnox. Bokför och betala manuellt under Leverantörsfakturor.`;
            } else {
              // Customer payment
              try {
                toolResult = await callFortnoxWrite(
